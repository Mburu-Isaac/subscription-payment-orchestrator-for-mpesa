...
def signup(): # add verification for signup - reinforce authentication at signup?
    if request.method == "POST":
        ...
        user_records = db.session.execute(
            db.select(User)
        ).scalars().all()
...
def account_recovery():
    ...
    password_records = db.session.execute(
        db.select(PasswordHistory).where(
    ...
    if request.method == "POST":
        ...
        for password in password_records:
            try:
                ...
                flash("password previously used. create a new password") # inform them that the password they added has been used previously
                # advise them to use a unique password
                return redirect(url_for("auth.account_recovery", user_id=user.id))
...
def get_email():
    ...
    if request.method == "POST":
        ...
        if not user:
            flash("email does not exist. confirm that the email entered is correct")
        ...
        if send_verification_email(otp,email):
            return redirect(url_for("auth.otp_verification", user_id=user.id))
    ...
    if request.method == "POST":
        ...
        if otp_cleanup():
            if datetime.utcnow() > otp_record.expires_at:
                ...
                flash("OTP expired. enter email to regenerate code")
                return redirect(url_for("auth.get_email")) # inform user OTP expired

            if otp_record.attempts_left <= 0: # subtract from attempts left
                try:
                    db.session.delete(otp_record)
                    db.session.commit()
            ...
            if check_password_hash(otp_record.hashed_otp, request.form.get("otp")):
                ...
                try:
                    ...
                    db.session.commit()
                    flash(f"invalid OTP. You have {otp_record.attempts_left} attempts left" ) # inform user on the webpage
                    return redirect(url_for("auth.otp_verification", user_id=user.id))
                except Exception:
                    db.session.rollback()
                    raise

    # get user email to send OTP to
...
# create product-ready dashboard